...
var numberPredicates = ["value", "quantityDispensed", "dispenseDaysSuppy"];
function CastNumbers(path, node){
  if (path.length === o) {
    return;
  }
  var here = path[path.length-1];
  numberPredicates.forEach(function(p){
    if (p === here){
...
var ClinicalStatementTypes = [
  "Medication", "VitalSignSet", "Encounter", "Fulfillment"
];
...
domains.forEach(function(a, i){
  fixersToJson.push(RemoveTypes.apply(null, a));
});
        ...
        if (smartContext[k] && smartContext[k]["@type"] == "@id" && 
          ...
          follow === k && path.length === 1
            ...
            if (typeof node === "string" && Object.keys(index[node]).length > 1){
              console.log("Could plug", index[node]);
...
module.exports =  function convert(params, callback){
    ...
    jsonld.fromRDF(params.body,
      ...
      function(err, js){
        jsonld.flatten(js, smartContext, function(err, js){
          console.log("Call #", i++, err);
          console.trace();
          if(err) return;
          console.log("flatten err", jsonString(err));
  ...
  if ( params.from === 'application/json' && params.to === 'application/rdf+xml' ){
    ...
    traverse(all(fixersToXml))(input['@graph']);
